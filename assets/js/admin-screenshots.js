...
(function($) {
	$('body').on('click','#admin-screenshots-popup .admin-screenshots-close-button',function(){
		...
		jQuery('#wp-admin-bar-admin-screenshots-button a').show();		
	});
	...
	$('#wp-admin-bar-admin-screenshots-button').on('click','a',function(se){
		screenshotThis();
		se.preventDefault();
		$(this).html('Creating screenshot...').attr('disabled','disabled');
	});
	...
	html2canvas(document.body, {
		...
		jQuery.ajax({
			...
			success: function(response) {
					...
					jQuery('#wp-admin-bar-admin-screenshots-button a').html('SCREENSHOT THIS PAGE').hide();
			},
